...
- **Presence web app channel**: optional `[presence]` block in an agent's `config.toml` (`enabled`, `address`, `port`, `static_dir`). When enabled, the daemon starts an `aiohttp` server *in the same event loop*, sharing the one `AgentSession` and `turn_lock` with Telegram + scheduler — so the web app and Telegram are the same agent, one continuous context/memory, and all channels serialise through the same lock. Serves a static frontend (`static_dir`) plus `POST /api/chat` (SSE): takes the last user message, runs `runtime.turn(..., extra_system=PRESENCE_INSTRUCTION)` (the agent's real model/identity/memory), peels the leading `<presence>{valence,energy,focus,note}</presence>` side-channel block off the reply, and emits `presence` / `token` / `done` / `error` events. `runtime.turn` gained an `extra_system` param (channel-specific instruction appended for one turn, not persisted). Non-streaming for now (full reply → one `presence` + one `token`); true token streaming would need a streaming path through the providers. The frontend + backend contract were authored by the agent Arden himself (`agents/arden/code/RA-chat-app/`, see its `CONTRACT.md`); this server is the real backend that retires his dev `server.js` shim. Bind to a tailnet IP like the GUI — there is no auth.
